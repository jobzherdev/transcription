...
## 8. Итоговый список по полноте охвата (минимум без потерь)

Ниже — минимальный набор, дающий **100 % покрытия** (проверено: 0 непокрытых записей из полного объединения). Избыточные списки можно не подключать — всё, что в них есть, уже покрыто оставшимися.

### 8.1. ДОМЕНЫ — оставить 6 списков (объединение = 2 561 уникальное имя)

| Список | Записей | Уникального вклада | Решение |
|---|--:|--:|---|
| `podkop-foreign-domains.lst` | 1417 | 1004 | ✅ оставить |
| `russia_inside.srs` | 1183 | 1068 | ✅ оставить |
| `google_ai.srs` | 28 | 27 | ✅ оставить |
| `google_play.srs` | 12 | 12 | ✅ оставить |
| `twitch-ad-bypass.srs` | 4 | 2 | ✅ оставить |
| `hodca.srs` | 250 | 1 | ✅ оставить (ради 1 имени; иначе можно добавить его вручную) |
| `discord.srs` | 20 | 0 | ❌ убрать — полностью в `russia_inside.srs` |
| `cloudflare.srs` (домены) | 4 | 0 | ❌ убрать — покрыт |
| `whatsapp-domains.txt` | 4 | 0 | ❌ убрать — покрыт |

### 8.2. IP — оставить 6 источников (объединение покрывает все ~17,4 тыс. + специфику)

| Список | Записей | Уникального вклада | Решение |
|---|--:|--:|---|
| `all-in-one.srs` | 17 376 | 14 564 | ✅ оставить (ядро) |
| `podkop-foreign-subnets.lst` | 10 498 | 1 875 | ✅ оставить |
| `cloudfront.srs` | 179 | 4 | ✅ оставить (ради 4 диапазонов) |
| `discord.srs` (IP) | 8 | 5 | ✅ оставить |
| `telegram-ipv4.lst` | 10 | 4 | ✅ оставить |
| `whatsapp-cidr_ipv4.txt` | 15 | 3 | ✅ оставить |
| `digitalocean.srs` | 164 | 0 | ❌ убрать — 100 % в `all-in-one` |
| `hetzner.srs` | 79 | 0 | ❌ убрать — 100 % в `all-in-one` |
| `cloudflare.srs` (IP) | 15 | 0 | ❌ убрать — покрыт |

### 8.3. Отдельные записи из задания

- **Домены (13 шт.):** ✅ все уникальны — добавить все.
- **IP:**
  - ✅ `91.108.0.0/16`, `184.66.194.18`, `93.189.201.43` — добавить (но `91.108.0.0/16` сделает избыточными мелкие telegram-подсети `91.108.*` — либо /16, либо мелкие, не оба).
  - ❌ `74.125.250.0/24`, `74.125.247.128/32` — **не добавлять**: оба уже покрыты `74.125.0.0/16` из `all-in-one.srs`.

### Итог

- **Домены:** `podkop-foreign-domains.lst` + `russia_inside.srs` + `google_ai.srs` + `google_play.srs` + `twitch-ad-bypass.srs` + `hodca.srs` + 13 отдельных доменов.
- **IP:** `all-in-one.srs` + `podkop-foreign-subnets.lst` + `cloudfront.srs` + `discord.srs` + `telegram-ipv4.lst` + `whatsapp-cidr_ipv4.txt` + (`91.108.0.0/16`, `184.66.194.18`, `93.189.201.43`).
- **Убрать как полностью избыточные:** `discord.srs`(домены), `cloudflare.srs`(домены+IP), `whatsapp-domains.txt`, `digitalocean.srs`, `hetzner.srs`.
